...
def parse_args():
    """解析命令行参数"""
    parser = argparse.ArgumentParser(description="ParaView结果处理工具")
    ...
    # 导出配置参数
    parser.add_argument('--target-times', type=float, nargs='+', default=[4.0, 5.0, 6.0],
                        help='导出的目标时间点，例如 --target-times 4.0 5.0 6.0')
    parser.add_argument('--output-dir-name', default='post_results',
                        help='输出目录名称')
    parser.add_argument('--image-width', type=int, default=1642,
                        help='输出图像宽度')
    parser.add_argument('--image-height', type=int, default=1083,
                        help='输出图像高度')
    
    # 导出字段参数
    parser.add_argument('--fields', nargs='+', 
                        default=['T:温度', 'hoop_stress:环向应力', 
                                'thermal_hoop_strain:环向热应变', 
                                'mechanical_hoop_strain:环向机械应变'],
                        help='导出字段列表，格式为 字段名:显示名 字段名:显示名 ...')
...
def main():
    ...
    for field_spec in args.fields:
        ...
        if len(parts) == 2:
            field_list.append((parts[0], parts[1]))
        else:
            field_list.append((field_spec, field_spec))
    ...
    # 找到所有.e文件
    e_files = find_e_files(studies_dir)
    if not e_files:
        print(f"未找到.e文件在: {studies_dir}")
        return
    ...
    for idx, e_file in enumerate(e_files):
        case_dir = os.path.dirname(e_file)
        case_name = os.path.basename(case_dir)
        print(f"\n处理文件 {idx+1}/{len(e_files)}: {case_name}/{os.path.basename(e_file)}")
        if export_field_data(e_file, config):
            success_count += 1
