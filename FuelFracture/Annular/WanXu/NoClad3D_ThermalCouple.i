...
pellet_thermal_expansion_coef=1.8e-5#K-1

# mpirun -n 16 /home/yp/projects/raccoon/raccoon-opt -i NoClad3D_ThermalCouple.i 
...
#mpirun -n 10 /home/yp/projects/raccoon/raccoon-opt -i step1.i --mesh-only KAERI_HANARO_UpperRod1_NoClad3D.e
#《《下面数据取自[1]Thermomechanical Analysis and Irradiation Test of Sintered Dual-Cooled Annular pellet》》

# 双冷却环形燃料几何参数 (单位：mm)(无内外包壳)
pellet_inner_diameter = 90         # 芯块内直径
pellet_outer_diameter = 100         # 芯块外直径
length = 0.05                    # 轴向长度(m)
# 网格控制参数n_azimuthal = 512时网格尺寸为6.8e-5m
n_radial_pellet = 8          # 燃料径向单元数
n_azimuthal = 64           # 周向基础单元数
...
#自适应法线公差
normal_tol = '${fparse 3.14*pellet_inner_diameter/n_azimuthal*1e-3/100}'
[Mesh]
  [pellet1]
...
[BCs]
  ...
  [inner_temp]
    ...
    function = temperature_cycle
  []
  [z_zero_on_bottom_top2]
    type = DirichletBC
    variable = T
...
[Functions]
  [temperature_cycle]
    type = ParsedFunction
    value = 'if(t<20, 100, 100 + 200*sin(0.1*pi*(t-20)))'
    # 前20秒保持100K，之后开始循环变化
  []
[]