...
# mpiexec -n 16 ../../raccoon-opt -i step4_AddCreep.i --n-threads=1 --timing
#几何与网格参数
...
pellet_specific_heat=300 # 特别注意一下文献中的单位与公式中使用的单位/(kg·K)
...
[Functions]
  ...
  [dt_limit_func]
    type = ParsedFunction
    expression = 'if(t <1,0.35,
                       if(abs(d_increment) < 1e-3,5000, 
                         if(abs(d_increment) < 5e-3,2000, 
                           if(abs(d_increment) < 1e-2,1000, 
                             if(abs(d_increment) < 5e-2,500, 
                               if(abs(d_increment) < 1e-1,250,100))))))'
...
[Outputs]
  ...
  []
  file_base = 'outputs/PF1_creep_mod/h=${h}_l=${length_scale_paramete}_Gc=${pellet_critical_energy}'
[]
