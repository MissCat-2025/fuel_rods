...
[Executioner]
  ...
  # 2.直接求解器（最快）
  #   petsc_options_iname = '-pc_type -pc_factor_mat_solver_package'#第二步很慢，但一直能运行收敛
  # petsc_options_value = 'lu       superlu_dist                 '
    # petsc_options_iname = '-pc_type'
    # petsc_options_value = 'lu'       # LU分解，小问题效果好
  #3.加速收敛，（巨慢，第一步倒是收敛了）中等规模 第一步能收敛
    petsc_options_iname = '-pc_type -sub_pc_type -sub_pc_factor_shift_type'
    petsc_options_value = 'asm lu NONZERO'  # 加速收敛
  #4.多重网格（第二快）大规模
    # petsc_options_iname = '-pc_type -pc_hypre_type' 第一步无法收敛
    # petsc_options_value = 'hypre boomeramg'  # 代数多重网格，大问题效果好
  # 5.GMRES重启参数（并列第二快）大规模问题 第一步无法收敛
