...
#重启：mpiexec -n 16 ../../raccoon-opt -i step4_AddCreep.i --n-threads=1 --timing --recover
#几何与网格参数
...
[MultiApps]
  [fracture]
    ...
    input_files = 'step4_AddCreep_sub.i'
  ...
  [dt_limit_func]
    ...
    expression = 'if(t <0.3,0.1,
                    if(t <105000,
                      ...
                      if(abs(d_increment) < 1e-3,500, 
                        if(abs(d_increment) < 5e-3,250, 
                          if(abs(d_increment) < 1e-2,200, 
                            if(abs(d_increment) < 5e-2,100, 
                              if(abs(d_increment) < 1e-1,50,25)))))))'
...
[Outputs]
  ...
  file_base = 'outputs/PF1_20250212_creep/h=${h}_l=${length_scale_paramete}_Gc=${pellet_critical_energy}_fission_ratedown'
[]
