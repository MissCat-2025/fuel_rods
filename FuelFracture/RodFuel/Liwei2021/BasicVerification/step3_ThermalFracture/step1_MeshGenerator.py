...
# 基础配置
base_dir = '/home/yp/projects/raccoon/FuelFracture/RodFuel/Liwei2021/BasicVerification/step3_ThermalFracture'
# 主程序模板文件与子程序模板文件相同，则是单程序模式
template_main = os.path.join(base_dir, 'NoClad3D_quarter_ThermallFractureNewH.i')
template_sub = os.path.join(base_dir, 'NoClad3D_quarter_ThermallFractureNewH_Sub.i')
...
parameter_matrix = {
    'Gf': [1,2.5,5,10],
    'length_scale_paramete': [1e-5,2.5e-5,5e-5,10e-5],
}

# 排除列表：已知无法运行的参数组合
# 格式：每个元组包含参数名和对应的值
exclude_combinations = [
    ('Gf', 1, 'length_scale_paramete', 10e-5),  # 示例：排除 Gf=8, length_scale_paramete=6e-5 的组合
    ('Gf', 10, 'length_scale_paramete', 1e-5),  # 示例：排除另一个组合
    ('Gf', 10, 'length_scale_paramete', 2.5e-5),  # 示例：排除另一个组合
]
