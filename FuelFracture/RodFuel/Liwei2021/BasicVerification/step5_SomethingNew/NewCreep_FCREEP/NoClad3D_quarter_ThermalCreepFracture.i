...
pellet_elastic_constants=2.1e11#Pa
pellet_nu = 0.345
...
[MultiApps]
  [fracture]
    ...
    input_files = 'NoClad3D_quarter_ThermalCreepFracture_Sub.i'
...
[Materials]
    ...
    expression = '(100/(7.5408 + 17.692*T/1000 + 3.6142*(T/1000)^2) + 6400/((T/1000)^2.5)*exp(-16.35/(T/1000)))*(1-0.2*d)'
    block = pellet
    []
    ...
    [total_power]
      ...
      expression = 'P * (1-0.3*d)'  # 直接使用函数符号进行计算
    ...
    # # # # 蠕变相关
    [creep]
      type = UO2CreepRateFCREEP
      # 必须参数
      temperature = T          # 耦合温度场
      oxygen_ratio = x        # 耦合氧化学计量比场
      fission_rate = 1.5e19                # 裂变率 (fissions/m³-s)
      vonMisesStress = vonMises   # von Mises应力场
      
      # 可选参数（带默认值）
      theoretical_density = 95.0         # 理论密度百分比
      grain_size = 10.0                  # 晶粒尺寸 (μm)
      gas_constant = 8.314               # 气体常数 (J/mol-K)
      initial_max_stress = 1e6           # 初始最大应力 (Pa)
      initial_max_stress_time = 0.0      # 初始最大应力时间 (s)
    []
    # 蠕变特征应变
    [creep_eigenstrain]
