...
1. 打开任意一个文件夹，在左边选择中下载这么一个插件wsl。(如下图)

![1742203175110](image/README/1742203175110.png)

2. 然后安装好后，按顺序同时按住shift+ctrl+p键呼出命令面板，输入wsl，选择连接到wsl（如下图），等待一会
...
回到Cursor在左端右键任意一个文件，选择在集成终端打开（如下图操作2），出现3号红色矩形的内容，这个

**《在集成终端打开》功能是非常非常非常好用的功能，请务必记住。如下图操作2**
...
将你下载的文件复制黏贴放到下面的位置即可(如下图，红色圈圈下面的文件就是咱们需要的Miniforge3-Linux-x86_64.sh)
